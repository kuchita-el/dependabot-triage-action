# 実装ロードマップ（初期分解スナップショット）

> **このファイルは更新しない静的文書。** 各 PR の進捗・ステータスの正は GitHub Milestones / Issues にあり、
> 本ファイルはそれを複製しない。ここに記すのは着手前に行った PR の初期分解（関心・所有ファイル・依存）のみ。
> スコープが動いた場合の現況は Milestones / Issues を参照すること。
...
| PR | 関心 | 所有ファイル | 依存 |
|---|---|---|---|
| A | プロジェクト雛形 | package.json / tsconfig / vitest / lint / action.yml枠 / ci.yml | — |
| B | 共有型・設定パース | types.ts / config.ts | — |
| C | スコア計算（純粋） | score.ts | B |
